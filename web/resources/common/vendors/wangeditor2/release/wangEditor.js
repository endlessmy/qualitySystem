...
// added by dyf (ie11 粘贴图片)
function getBase64SrcAndUpload(e, editor) {
    setTimeout(function() {
        var imgList = e.target.querySelectorAll('img');
        var srcStr = '';
        ...
        var blob = convertBase64UrlToBlob(srcStr);
        var uploadImg = editor.uploadImg;
        uploadImg.uploadImg([blob]);
    }, 1);
}
/*
    编辑区域
...
Text.prototype = {
    ...
    _pasteHandle: function _pasteHandle() {
        ...
        $textElem.on('paste', function (e) {
            if (UA.isIE()) {
                getBase64SrcAndUpload(e, editor)
                return;
            } else {
                e.preventDefault();
            }
...
Command.prototype = {
    ...
    do: function _do(name, value) {
        ...
        // 最后，恢复选取保证光标在原来的位置闪烁
        // @qqli 注释(如此加粗的功能有bug@dengyingfeng)
        // editor.selection.saveRange();
        // @qqli end
        editor.selection.saveRange();
    ...
    _insertHTML: function _insertHTML(html) {
        ...
        // add by dengyingfeng 2018/04/12
        if (~html.indexOf('<img') && range) {
            try {
