    ...
    <div>
        ...
        <el-table
            ...
            <el-table-column
                ...
                label="上线工单数">
                <template scope="scope">
                    <span v-if="scope.row.online_count">
                        {{ scope.row.online_count }}
                    </span>
                    <span v-else-if="scope.row.online_count===0">
                        {{ 0 }}
                    </span>
                    <span v-else>
                        {{ '--' }}
...
export default {
    ...
    computed: {
        ...mapState({
            tableListFinished: ({ qualitySystem }) => qualitySystem.kpiReachFinishedData.result,
            pageDataFinished: ({ qualitySystem }) => qualitySystem.kpiReachFinishedData.basePage,
            tableListVideoFinished: ({ qualitySystem }) => qualitySystem.kpiReachVideoFinishedData.result,
            pageDataVideoFinished: ({ qualitySystem }) => qualitySystem.kpiReachVideoFinishedData.basePage,
            // 用于负责人表头筛选
        ...
        tableList() {
            if (this.kpiData === 'finished') {
                ...
                return this.tableListVideoFinished;
            }
        },
        pageData() {
            if (this.kpiData === 'finished') {
                ...
                return this.pageDataVideoFinished;
            }
        },
        type() {
            return this.kpiData;
    ...
    watch: {
        ...
        kpiData() {
            this.clearParams(); // 清除筛选条件的请求参数
            this.$refs.sort.columnConfig.order = ''; // 清除排序箭头选中
            this.filterItem = []; // 清除筛选条件数组
            this.requestList();
        }
        // tableList(val) {
        //     if (val.length !== 0) {
    ...
    methods: {
        ...
        requestList(pageNo) {
            ...
            if (this.type === 'finished') {
                if (this.requestParams.status === '' || this.requestParams.year === '' || this.requestParams.quarter === '') {
                    ...
                    this.queryKpiReachFinishedData(this.formatParams());
                }
            } else if (this.type === 'videoFinished') {
                if (this.requestParams.status === '' || this.requestParams.year === '' || this.requestParams.quarter === '') {
                    return false;
                } else {
                    this.queryKpiReachVideoFinishedData(this.formatParams());
                }
            }
        ...
        ...mapActions(['queryKpiReachFinishedData', 'queryKpiReachVideoFinishedData'])
    }
};
</script>
